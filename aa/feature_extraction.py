    ...
    def samples(data):
        ...
        for i in n: 
            ...
            for i in word_ids:
                ...
                tensor_en = torch.Tensor(encode)
                a = torch.cat([tensor_en, tensor_en.new_zeros(word_max_length - tensor_en.size(0))],0) # pad the tensor for word to match max length of word
                sent_encode.append(a)
                

            b = torch.stack(sent_encode,0)             
            p=torch.cat([b,torch.zeros(max_sample_length - len(sent_encode),word_max_length)],0) # pad tensors for sentences to match max length of sentence
